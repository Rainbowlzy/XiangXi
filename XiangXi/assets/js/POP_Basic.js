require(['jquery', 'jquery.cookie', 'bootstrap', 'bootstrap-table', 'bootstrap-select', 'fileinput', 'fileinput_locale_zh'], function ($) {
    ...
    function process(obj) {
        ...
        if (isSanShanDao) {
            //在三山岛下面增加家庭管理对应的字段
            return obj.slice(0,obj.length - 1).concat([
                {
                    field: 'bookletType',
                    title: '户口类型',
                    sortable: true,
    ...
    var isSanShanDao = ($.cookie('JTZH_districtID') || "").startsWith('D1005031612');
    console.log(($.cookie('JTZH_districtID') || ""));
    console.log('D1005031612');
    console.log(isSanShanDao);
    ...
    window.operateEvents = {
        ...
        'click .edit': function (e, value, row, index) {
                 ...
                 '<form role="form" class="row form-horizontal">' +
                    '<div class="col-md-6">' +
                        '<div class="form-group">' +
                            '<label class="col-sm-3 control-label" for="name_edit">姓名</label>' +
                            '<div class="col-sm-9">' +
                        ...
                        '<div class="form-group">' +
                            ...
                            '<div class="col-sm-9">' +
                                ' <input class="form-control" type="text" id="district" placeholder="区域">' +                                
                                ' <input class="form-control" type="text" id="plot" placeholder="小区/组">' +
                                ' <input class="form-control" type="text" id="houseNum" placeholder="门牌号">' +
                       ...
                       '<div class="form-group">' +
                            ...
                            '<div class="col-sm-9">' +
                               ' <input class="form-control" type="text" id="bookletNum" placeholder="请输入户口本号" disabled>' +
                           ' </div>' +
                       ' </div>' +
                        '<div class="form-group">' +
                  ...
                  '  </div>' +
                    '<div class="col-lg-6">' +
                       ' <div class="form-group">' +
                           ...
                           ' <div class="col-sm-9">' +
                               ' <input class="form-control" type="text" id="censusRegister" placeholder="请输入户籍地址" disabled>' +
                           ' </div>' +
                       ' </div>' +
                       ' <div class="form-group">' +
                           ' <label class="col-sm-3 control-label" for="populationType">人口类型</label>' +
                          '  <div class="col-sm-9">' +
                              '  <select id="populationType" class="form-control selectpicker" title="人口类型" disabled>' +
            ...
            console.log(row,$('input:radio[name="relationship"]:checked').val(),row.name, row.IDCard, row.bookletNum, row.marriageStatus, row.censusRegister, row.plot, row.houseNum);
            $('#name').val(row.name);
            $('#IDCard').val(row.IDCard);
            $('#bookletNum').val(row.bookletNum);
            $('#phone').val(row.phone);
            $('#workPlace').val(row.workPlace);
            $('#censusRegister').val(row.censusRegister);

            $('#district').val(row.district);
            //类型默认选
            $('#politicsStatus').selectpicker('val', row.politicsStatus);
            $('#marriageStatus').selectpicker('val', row.marriageStatus);
            $('#educationDegree').selectpicker('val', row.educationDegree);
            $('#populationType').selectpicker('val', row.populationType);
            $('#nation').selectpicker('val', row.nation);
            if (row.sex == '男') {
            ...
            $('#table_edit_modal .btn-success').click(function () {
                $.ajax({
                    url: SVC_POP + "/editBasicPopulation",
                    type: "GET",
                    data: {
                        id: row.id,
                        name: $('#name').val(),
    ...
    $('#table-add').click(function () {
        ...
        $('#table_add_modal').on('shown.bs.modal', function () {
