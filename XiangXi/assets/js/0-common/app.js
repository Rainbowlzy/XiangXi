...
require.config({
    ...
    paths: {
        ...
        "jquery": ["../assets/js/0-common/third-party/jquery-1.10.2"],
        "bootstrap-table-locale-all": ["https://cdn.bootcss.com/bootstrap-table/1.10.0/bootstrap-table-locale-all"],
        "bootstrap-table-zh-CN": ["../assets/js/0-common/bootstrap-table-zh-CN.min"],
        ...
        "echarts": ['../assets/js/0-common/echarts-all'],
    ...
    shim: (_shim = {
        ueditor: {
            exports: 'ueditor',
            deps: [
                ,'../assets/UEditor-utf8-net/ueditor.config'
                , '../assets/js/0-common/third-party/zeroclipboard/ZeroClipboard'
                , '../assets/UEditor-utf8-net/lang/zh-cn/zh-cn'
            ]
        },
        echarts: {
            exports: 'echarts'
        ...
        jquery: {
            exports: 'jquery',
            //deps: ['../assets/js/jquery-3.1.0.js']
        },
        bootstrap: {
            deps: ['jquery']
        },
...
if (window.location.href.indexOf("localhost") !== -1) {
	...
	// svcHeader = location.href.substr(0,location.href.indexOf('/',10)).replace(/http[s]*:\/\//i,'')
}
...
function operateFormatter(value, row, index) {
    ...
    for (var p in row) {
        if (p.toLowerCase().indexOf('address') !== -1
            ...
            var k = p;
            buttons.push([
                '<a class=" position am-btn"   href="', '/XiangXi/1_index/index.html?data=' + encodeURIComponent(JSON.stringify(Object.assign(row,{
                    address:row[k]
                }))),
                '"  title="位置" >',
...
require(['jquery','jquery.cookie'],function(){
    ...
    window.Call = function Call(method, data, callback) {
        var params = $.extend({ data: encodeURIComponent(JSON.stringify(data)) }, data instanceof Array ? {} : {});
        $.get(SVC_NEW + "?method=" + method, params, function (resp) {
            if (!callback) {
                callback = function callback(one) {
                    console.log(one);
                };
            }
            if (callback) callback(resp);else console.error(resp);
        });
    };
    ...
    $.extend({
        ...
        setTimeoutQueue: function setTimeoutQueue(arr, n) {
        ...
        calcCenter: function calcCenter(points) {
        ...
        login: function login(user, pwd) {
            $.call('login', {
                ...
                if (!(data && data.success)) {
                    ...
                    if (location.href.indexOf('login.html') !== -1) location.href = '../1_index/index.html';
                }
            });
        },
        back2login: function back2login() {
            location.href = '/XiangXi/4_login/login.html';
        },
        select: function select(obj, props) {
            var o = {};
            for (var p in obj) {
        ...
        call: function call(method, data, callback) {
            ...
            var thiz = this;
            return Call(method, data, function (resp) {
                if (resp && !resp.success && resp.message && resp.message.indexOf('请登录') !== -1) _this.back2login();
                if (callback) callback(resp);
            });
        },
        callsync: function callsync(method, data) {
            var thiz = this;
            var text = $.ajax({
        ...
        mapSearch: function mapSearch(key, fn) {
