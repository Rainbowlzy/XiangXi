...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-FinancialManagement').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-FinancialManagement').on('shown.bs.modal',function(){
					$('.table_excel_modal-FinancialManagement .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=FinancialManagement", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"FinancialManagementList.html"
					});
				})

				$('.table_excel_modal-FinancialManagement').modal('show');
			})
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveFinancialManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"FinancialManagementList.html?table=FinancialManagement"
                });
            },

			back(){
				window.location.href = site_gen+"FinancialManagement.html?table=FinancialManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetFinancialManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'FinancialManagement'
					vm = new Vue({
						el: "#FinancialManagement",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'FinancialManagement'
				vm = new Vue({
					el: "#FinancialManagement",
					data: request||{},
					methods: method,
