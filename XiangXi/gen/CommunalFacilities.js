...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-CommunalFacilities').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-CommunalFacilities').on('shown.bs.modal',function(){
					$('.table_excel_modal-CommunalFacilities .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "/XiangXi/ExcelImport.ashx?fileType=CommunalFacilities", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"CommunalFacilitiesList.html"
					});
				})

				$('.table_excel_modal-CommunalFacilities').modal('show');
			})
			$("#CommunalFacilities").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveCommunalFacilities", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"CommunalFacilitiesList.html?table=CommunalFacilities"
                });
            },

			back(){
				window.location.href = site_gen+"CommunalFacilities.html?table=CommunalFacilities"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetCommunalFacilitiesEmpty", {}, function(data) {
					data = data||{};
					data.table = 'CommunalFacilities'
					vm = new Vue({
						el: "#CommunalFacilities",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'CommunalFacilities'
				vm = new Vue({
					el: "#CommunalFacilities",
					data: request||{},
					computed:{
