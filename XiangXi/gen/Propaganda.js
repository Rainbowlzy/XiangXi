    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePropaganda", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PropagandaList.html?table=Propaganda"
                });
            },

			back(){
				window.location.href = site_gen+"Propaganda.html?table=Propaganda"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPropagandaEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Propaganda'
					vm = new Vue({
						el: "#Propaganda",
						data: data||{},
						computed:{
