    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveProposedTreatment", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ProposedTreatmentList.html?table=ProposedTreatment"
                });
            },

			back(){
				window.location.href = site_gen+"ProposedTreatment.html?table=ProposedTreatment"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetProposedTreatmentEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ProposedTreatment'
					vm = new Vue({
						el: "#ProposedTreatment",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'ProposedTreatment'
				vm = new Vue({
					el: "#ProposedTreatment",
					data: request||{},
					computed:{
