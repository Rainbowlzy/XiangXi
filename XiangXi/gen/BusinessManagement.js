    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveBusinessManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"BusinessManagementList.html?table=BusinessManagement"
                });
            },

			back(){
				window.location.href = site_gen+"BusinessManagement.html?table=BusinessManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetBusinessManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'BusinessManagement'
					vm = new Vue({
						el: "#BusinessManagement",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'BusinessManagement'
				vm = new Vue({
					el: "#BusinessManagement",
					data: request||{},
					computed:{
