    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveUnitedFrontMilitia", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"UnitedFrontMilitiaList.html?table=UnitedFrontMilitia"
                });
            },

			back(){
				window.location.href = site_gen+"UnitedFrontMilitia.html?table=UnitedFrontMilitia"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetUnitedFrontMilitiaEmpty", {}, function(data) {
					data = data||{};
					data.table = 'UnitedFrontMilitia'
					vm = new Vue({
						el: "#UnitedFrontMilitia",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'UnitedFrontMilitia'
				vm = new Vue({
					el: "#UnitedFrontMilitia",
					data: request||{},
					computed:{
