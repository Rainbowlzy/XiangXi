    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveRoleMenu", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"RoleMenuList.html?table=RoleMenu"
                });
            },

			back(){
				window.location.href = site_gen+"RoleMenu.html?table=RoleMenu"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetRoleMenuEmpty", {}, function(data) {
					data = data||{};
					data.table = 'RoleMenu'
					vm = new Vue({
						el: "#RoleMenu",
