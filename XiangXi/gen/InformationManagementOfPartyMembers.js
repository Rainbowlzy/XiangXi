...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$("#InformationManagementOfPartyMembers").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveInformationManagementOfPartyMembers", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"InformationManagementOfPartyMembersList.html?table=InformationManagementOfPartyMembers"
                });
            },

			back(){
				window.location.href = site_gen+"InformationManagementOfPartyMembers.html?table=InformationManagementOfPartyMembers"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetInformationManagementOfPartyMembersEmpty", {}, function(data) {
					data = data||{};
					data.table = 'InformationManagementOfPartyMembers'
					vm = new Vue({
						el: "#InformationManagementOfPartyMembers",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'InformationManagementOfPartyMembers'
				vm = new Vue({
					el: "#InformationManagementOfPartyMembers",
					data: request||{},
					computed:{
