...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Information').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Information').on('shown.bs.modal',function(){
					$('.table_excel_modal-Information .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Information", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"InformationList.html"
					});
				})

				$('.table_excel_modal-Information').modal('show');
			})
			$("#Information").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveInformation", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"InformationList.html?table=Information"
                });
            },

			back(){
				window.location.href = site_gen+"Information.html?table=Information"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetInformationEmpty", {}, function(data) {
					data = data||{};
