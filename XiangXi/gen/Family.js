    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveFamily", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"FamilyList.html?table=Family"
                });
            },

			back(){
				window.location.href = site_gen+"Family.html?table=Family"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetFamilyEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Family'
					vm = new Vue({
						el: "#Family",
