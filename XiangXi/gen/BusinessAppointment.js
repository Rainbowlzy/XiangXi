    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveBusinessAppointment", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"BusinessAppointmentList.html?table=BusinessAppointment"
                });
            },

			back(){
				window.location.href = site_gen+"BusinessAppointment.html?table=BusinessAppointment"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetBusinessAppointmentEmpty", {}, function(data) {
					data = data||{};
					data.table = 'BusinessAppointment'
					vm = new Vue({
						el: "#BusinessAppointment",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'BusinessAppointment'
				vm = new Vue({
					el: "#BusinessAppointment",
					data: request||{},
					computed:{
