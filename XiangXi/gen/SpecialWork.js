    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveSpecialWork", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SpecialWorkList.html?table=SpecialWork"
                });
            },

			back(){
				window.location.href = site_gen+"SpecialWork.html?table=SpecialWork"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSpecialWorkEmpty", {}, function(data) {
					data = data||{};
					data.table = 'SpecialWork'
					vm = new Vue({
						el: "#SpecialWork",
						data: data||{},
						computed:{
