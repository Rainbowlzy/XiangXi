...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Engineer').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Engineer').on('shown.bs.modal',function(){
					$('.table_excel_modal-Engineer .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Engineer", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"EngineerList.html"
					});
				})

				$('.table_excel_modal-Engineer').modal('show');
			})
			$("#Engineer").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveEngineer", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"EngineerList.html?table=Engineer"
                });
            },

			back(){
				window.location.href = site_gen+"Engineer.html?table=Engineer"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetEngineerEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Engineer'
					vm = new Vue({
						el: "#Engineer",
