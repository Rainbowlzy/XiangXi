    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveWelfarePayment", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"WelfarePaymentList.html?table=WelfarePayment"
                });
            },

			back(){
				window.location.href = site_gen+"WelfarePayment.html?table=WelfarePayment"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetWelfarePaymentEmpty", {}, function(data) {
					data = data||{};
					data.table = 'WelfarePayment'
					vm = new Vue({
						el: "#WelfarePayment",
						data: data||{},
						computed:{
