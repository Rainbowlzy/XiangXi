...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$("#MedicalInsuranceForChildren").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveMedicalInsuranceForChildren", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"MedicalInsuranceForChildrenList.html?table=MedicalInsuranceForChildren"
                });
            },

			back(){
				window.location.href = site_gen+"MedicalInsuranceForChildren.html?table=MedicalInsuranceForChildren"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetMedicalInsuranceForChildrenEmpty", {}, function(data) {
					data = data||{};
					data.table = 'MedicalInsuranceForChildren'
					vm = new Vue({
						el: "#MedicalInsuranceForChildren",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'MedicalInsuranceForChildren'
				vm = new Vue({
					el: "#MedicalInsuranceForChildren",
					data: request||{},
					computed:{
