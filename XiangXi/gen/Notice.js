    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveNotice", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"NoticeList.html?table=Notice"
                });
            },

			back(){
				window.location.href = site_gen+"Notice.html?table=Notice"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetNoticeEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Notice'
					vm = new Vue({
						el: "#Notice",
