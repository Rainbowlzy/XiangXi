    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveDocumentManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"DocumentManagementList.html?table=DocumentManagement"
                });
            },

			back(){
				window.location.href = site_gen+"DocumentManagement.html?table=DocumentManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetDocumentManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'DocumentManagement'
					vm = new Vue({
						el: "#DocumentManagement",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'DocumentManagement'
				vm = new Vue({
					el: "#DocumentManagement",
					data: request||{},
					computed:{
