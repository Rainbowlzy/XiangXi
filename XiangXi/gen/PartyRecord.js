    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePartyRecord", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PartyRecordList.html?table=PartyRecord"
                });
            },

			back(){
				window.location.href = site_gen+"PartyRecord.html?table=PartyRecord"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPartyRecordEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PartyRecord'
					vm = new Vue({
						el: "#PartyRecord",
						data: data||{},
						computed:{
