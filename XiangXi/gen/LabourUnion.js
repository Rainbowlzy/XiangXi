    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveLabourUnion", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"LabourUnionList.html?table=LabourUnion"
                });
            },

			back(){
				window.location.href = site_gen+"LabourUnion.html?table=LabourUnion"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetLabourUnionEmpty", {}, function(data) {
					data = data||{};
					data.table = 'LabourUnion'
					vm = new Vue({
						el: "#LabourUnion",
						data: data||{},
						computed:{
