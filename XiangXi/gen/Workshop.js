...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Workshop').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Workshop').on('shown.bs.modal',function(){
					$('.table_excel_modal-Workshop .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Workshop", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"WorkshopList.html"
					});
				})

				$('.table_excel_modal-Workshop').modal('show');
			})
			$("#Workshop").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveWorkshop", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"WorkshopList.html?table=Workshop"
                });
            },

			back(){
				window.location.href = site_gen+"Workshop.html?table=Workshop"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetWorkshopEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Workshop'
					vm = new Vue({
						el: "#Workshop",
