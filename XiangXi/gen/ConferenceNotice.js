...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-ConferenceNotice').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-ConferenceNotice').on('shown.bs.modal',function(){
					$('.table_excel_modal-ConferenceNotice .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=ConferenceNotice", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"ConferenceNoticeList.html"
					});
				})

				$('.table_excel_modal-ConferenceNotice').modal('show');
			})
			$("#ConferenceNotice").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveConferenceNotice", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ConferenceNoticeList.html?table=ConferenceNotice"
                });
            },

			back(){
				window.location.href = site_gen+"ConferenceNotice.html?table=ConferenceNotice"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetConferenceNoticeEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ConferenceNotice'
					vm = new Vue({
						el: "#ConferenceNotice",
						data: data||{},
						methods: method,
				...
				request.table = 'ConferenceNotice'
				vm = new Vue({
					el: "#ConferenceNotice",
					data: request||{},
					methods: method,
