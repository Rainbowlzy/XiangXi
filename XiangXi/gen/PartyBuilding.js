    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePartyBuilding", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PartyBuildingList.html?table=PartyBuilding"
                });
            },

			back(){
				window.location.href = site_gen+"PartyBuilding.html?table=PartyBuilding"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPartyBuildingEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PartyBuilding'
					vm = new Vue({
						el: "#PartyBuilding",
						data: data||{},
						computed:{
