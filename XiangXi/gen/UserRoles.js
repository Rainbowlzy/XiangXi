    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveUserRoles", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"UserRolesList.html?table=UserRoles"
                });
            },

			back(){
				window.location.href = site_gen+"UserRoles.html?table=UserRoles"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetUserRolesEmpty", {}, function(data) {
					data = data||{};
