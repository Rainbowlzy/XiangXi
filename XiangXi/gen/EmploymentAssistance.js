    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveEmploymentAssistance", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"EmploymentAssistanceList.html?table=EmploymentAssistance"
                });
            },

			back(){
				window.location.href = site_gen+"EmploymentAssistance.html?table=EmploymentAssistance"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetEmploymentAssistanceEmpty", {}, function(data) {
					data = data||{};
					data.table = 'EmploymentAssistance'
					vm = new Vue({
						el: "#EmploymentAssistance",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'EmploymentAssistance'
				vm = new Vue({
					el: "#EmploymentAssistance",
					data: request||{},
					computed:{
