...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-MonitoringList').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-MonitoringList').on('shown.bs.modal',function(){
					$('.table_excel_modal-MonitoringList .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=MonitoringList", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"MonitoringListList.html"
					});
				})

				$('.table_excel_modal-MonitoringList').modal('show');
			})
			$("#MonitoringList").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveMonitoringList", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"MonitoringListList.html?table=MonitoringList"
                });
            },

			back(){
				window.location.href = site_gen+"MonitoringList.html?table=MonitoringList"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetMonitoringListEmpty", {}, function(data) {
					data = data||{};
					data.table = 'MonitoringList'
					vm = new Vue({
						el: "#MonitoringList",
						data: data||{},
						methods: method,
