    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveDangerousHousing", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"DangerousHousingList.html?table=DangerousHousing"
                });
            },

			back(){
				window.location.href = site_gen+"DangerousHousing.html?table=DangerousHousing"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetDangerousHousingEmpty", {}, function(data) {
					data = data||{};
					data.table = 'DangerousHousing'
					vm = new Vue({
						el: "#DangerousHousing",
						data: data||{},
						computed:{
				...
				request.table = 'DangerousHousing'
				vm = new Vue({
					el: "#DangerousHousing",
					data: request||{},
					computed:{
