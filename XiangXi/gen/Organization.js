    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveOrganization", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"OrganizationList.html?table=Organization"
                });
            },

			back(){
				window.location.href = site_gen+"Organization.html?table=Organization"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetOrganizationEmpty", {}, function(data) {
					data = data||{};
