    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveCadre", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"CadreList.html?table=Cadre"
                });
            },

			back(){
				window.location.href = site_gen+"Cadre.html?table=Cadre"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetCadreEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Cadre'
					vm = new Vue({
						el: "#Cadre",
