    ...
    $(document).ready(function() {
			...
			$('.excel-import-button-RoleConfiguration').click(function () {
				...
				$('.table_excel_modal-RoleConfiguration').on('shown.bs.modal',function(){
					...
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "/XiangXi/ExcelImport.ashx?fileType=RoleConfiguration", //上传的地址
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveRoleConfiguration", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"RoleConfigurationList.html?table=RoleConfiguration"
                });
            },

			back(){
				window.location.href = site_gen+"RoleConfiguration.html?table=RoleConfiguration"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetRoleConfigurationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'RoleConfiguration'
					vm = new Vue({
						el: "#RoleConfiguration",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'RoleConfiguration'
				vm = new Vue({
					el: "#RoleConfiguration",
					data: request||{},
					computed:{
