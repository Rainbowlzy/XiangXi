...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Sneakers').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Sneakers').on('shown.bs.modal',function(){
					$('.table_excel_modal-Sneakers .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Sneakers", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"SneakersList.html"
					});
				})

				$('.table_excel_modal-Sneakers').modal('show');
			})
			$("#Sneakers").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveSneakers", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SneakersList.html?table=Sneakers"
                });
            },

			back(){
				window.location.href = site_gen+"Sneakers.html?table=Sneakers"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSneakersEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Sneakers'
					vm = new Vue({
						el: "#Sneakers",
