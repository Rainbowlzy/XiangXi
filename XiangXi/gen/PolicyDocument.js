    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePolicyDocument", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PolicyDocumentList.html?table=PolicyDocument"
                });
            },

			back(){
				window.location.href = site_gen+"PolicyDocument.html?table=PolicyDocument"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPolicyDocumentEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PolicyDocument'
					vm = new Vue({
						el: "#PolicyDocument",
						data: data||{},
						computed:{
