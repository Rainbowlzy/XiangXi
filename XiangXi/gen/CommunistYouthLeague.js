    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveCommunistYouthLeague", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"CommunistYouthLeagueList.html?table=CommunistYouthLeague"
                });
            },

			back(){
				window.location.href = site_gen+"CommunistYouthLeague.html?table=CommunistYouthLeague"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetCommunistYouthLeagueEmpty", {}, function(data) {
					data = data||{};
					data.table = 'CommunistYouthLeague'
					vm = new Vue({
						el: "#CommunistYouthLeague",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'CommunistYouthLeague'
				vm = new Vue({
					el: "#CommunistYouthLeague",
					data: request||{},
					computed:{
