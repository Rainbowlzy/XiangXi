...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Conference').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Conference').on('shown.bs.modal',function(){
					$('.table_excel_modal-Conference .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Conference", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"ConferenceList.html"
					});
				})

				$('.table_excel_modal-Conference').modal('show');
			})
			$("#Conference").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveConference", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ConferenceList.html?table=Conference"
                });
            },

			back(){
				window.location.href = site_gen+"Conference.html?table=Conference"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetConferenceEmpty", {}, function(data) {
					data = data||{};
