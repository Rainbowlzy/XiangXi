...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Framework').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Framework').on('shown.bs.modal',function(){
					$('.table_excel_modal-Framework .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Framework", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"FrameworkList.html"
					});
				})

				$('.table_excel_modal-Framework').modal('show');
			})
			$("#Framework").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveFramework", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"FrameworkList.html?table=Framework"
                });
            },

			back(){
				window.location.href = site_gen+"Framework.html?table=Framework"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetFrameworkEmpty", {}, function(data) {
					data = data||{};
