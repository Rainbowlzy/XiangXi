    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePhoto", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PhotoList.html?table=Photo"
                });
            },

			back(){
				window.location.href = site_gen+"Photo.html?table=Photo"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPhotoEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Photo'
					vm = new Vue({
						el: "#Photo",
