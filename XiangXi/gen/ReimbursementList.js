    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveReimbursementList", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ReimbursementListList.html?table=ReimbursementList"
                });
            },

			back(){
				window.location.href = site_gen+"ReimbursementList.html?table=ReimbursementList"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetReimbursementListEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ReimbursementList'
					vm = new Vue({
						el: "#ReimbursementList",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'ReimbursementList'
				vm = new Vue({
					el: "#ReimbursementList",
					data: request||{},
					computed:{
