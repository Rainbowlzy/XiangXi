    ...
    $(document).ready(function() {
			...
			$('.excel-import-button-MenuConfiguration').click(function () {
				...
				$('.table_excel_modal-MenuConfiguration').on('shown.bs.modal',function(){
					...
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "/XiangXi/ExcelImport.ashx?fileType=MenuConfiguration", //上传的地址
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveMenuConfiguration", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"MenuConfigurationList.html?table=MenuConfiguration"
                });
            },

			back(){
				window.location.href = site_gen+"MenuConfiguration.html?table=MenuConfiguration"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetMenuConfigurationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'MenuConfiguration'
					vm = new Vue({
						el: "#MenuConfiguration",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'MenuConfiguration'
				vm = new Vue({
					el: "#MenuConfiguration",
					data: request||{},
					computed:{
