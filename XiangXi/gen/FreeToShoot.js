    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveFreeToShoot", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"FreeToShootList.html?table=FreeToShoot"
                });
            },

			back(){
				window.location.href = site_gen+"FreeToShoot.html?table=FreeToShoot"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetFreeToShootEmpty", {}, function(data) {
					data = data||{};
					data.table = 'FreeToShoot'
					vm = new Vue({
						el: "#FreeToShoot",
						data: data||{},
						computed:{
