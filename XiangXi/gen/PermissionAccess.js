    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePermissionAccess", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PermissionAccessList.html?table=PermissionAccess"
                });
            },

			back(){
				window.location.href = site_gen+"PermissionAccess.html?table=PermissionAccess"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPermissionAccessEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PermissionAccess'
					vm = new Vue({
						el: "#PermissionAccess",
						data: data||{},
						computed:{
				...
				request.table = 'PermissionAccess'
				vm = new Vue({
					el: "#PermissionAccess",
					data: request||{},
					computed:{
