...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-SanitationPersonnel').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-SanitationPersonnel').on('shown.bs.modal',function(){
					$('.table_excel_modal-SanitationPersonnel .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=SanitationPersonnel", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"SanitationPersonnelList.html"
					});
				})

				$('.table_excel_modal-SanitationPersonnel').modal('show');
			})
			$("#SanitationPersonnel").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveSanitationPersonnel", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SanitationPersonnelList.html?table=SanitationPersonnel"
                });
            },

			back(){
				window.location.href = site_gen+"SanitationPersonnel.html?table=SanitationPersonnel"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSanitationPersonnelEmpty", {}, function(data) {
					data = data||{};
					data.table = 'SanitationPersonnel'
					vm = new Vue({
						el: "#SanitationPersonnel",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'SanitationPersonnel'
				vm = new Vue({
					el: "#SanitationPersonnel",
					data: request||{},
					methods: method,
