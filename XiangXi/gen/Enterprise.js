...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-Enterprise').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-Enterprise').on('shown.bs.modal',function(){
					$('.table_excel_modal-Enterprise .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=Enterprise", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"EnterpriseList.html"
					});
				})

				$('.table_excel_modal-Enterprise').modal('show');
			})
			$("#Enterprise").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveEnterprise", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"EnterpriseList.html?table=Enterprise"
                });
            },

			back(){
				window.location.href = site_gen+"Enterprise.html?table=Enterprise"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetEnterpriseEmpty", {}, function(data) {
					data = data||{};
