    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePersonalInformation", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PersonalInformationList.html?table=PersonalInformation"
                });
            },

			back(){
				window.location.href = site_gen+"PersonalInformation.html?table=PersonalInformation"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPersonalInformationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PersonalInformation'
					vm = new Vue({
				...
				request.table = 'PersonalInformation'
				vm = new Vue({
					el: "#PersonalInformation",
					data: request||{},
					computed:{
