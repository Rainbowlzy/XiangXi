    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveHouseProperty", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"HousePropertyList.html?table=HouseProperty"
                });
            },

			back(){
				window.location.href = site_gen+"HouseProperty.html?table=HouseProperty"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetHousePropertyEmpty", {}, function(data) {
					data = data||{};
					data.table = 'HouseProperty'
					vm = new Vue({
						el: "#HouseProperty",
						data: data||{},
						computed:{
