    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveBusinessGuide", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"BusinessGuideList.html?table=BusinessGuide"
                });
            },

			back(){
				window.location.href = site_gen+"BusinessGuide.html?table=BusinessGuide"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetBusinessGuideEmpty", {}, function(data) {
					data = data||{};
					data.table = 'BusinessGuide'
					vm = new Vue({
						el: "#BusinessGuide",
						data: data||{},
						computed:{
