...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-SecurityPatrolRecord').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-SecurityPatrolRecord').on('shown.bs.modal',function(){
					$('.table_excel_modal-SecurityPatrolRecord .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=SecurityPatrolRecord", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"SecurityPatrolRecordList.html"
					});
				})

				$('.table_excel_modal-SecurityPatrolRecord').modal('show');
			})
			$("#SecurityPatrolRecord").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveSecurityPatrolRecord", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SecurityPatrolRecordList.html?table=SecurityPatrolRecord"
                });
            },

			back(){
				window.location.href = site_gen+"SecurityPatrolRecord.html?table=SecurityPatrolRecord"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSecurityPatrolRecordEmpty", {}, function(data) {
					data = data||{};
					data.table = 'SecurityPatrolRecord'
					vm = new Vue({
						el: "#SecurityPatrolRecord",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'SecurityPatrolRecord'
				vm = new Vue({
					el: "#SecurityPatrolRecord",
					data: request||{},
					methods: method,
