    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveServiceAppointment", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ServiceAppointmentList.html?table=ServiceAppointment"
                });
            },

			back(){
				window.location.href = site_gen+"ServiceAppointment.html?table=ServiceAppointment"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetServiceAppointmentEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ServiceAppointment'
					vm = new Vue({
				...
				request.table = 'ServiceAppointment'
				vm = new Vue({
					el: "#ServiceAppointment",
					data: request||{},
					computed:{
