    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveMailList", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"MailListList.html?table=MailList"
                });
            },

			back(){
				window.location.href = site_gen+"MailList.html?table=MailList"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetMailListEmpty", {}, function(data) {
					data = data||{};
					data.table = 'MailList'
					vm = new Vue({
						el: "#MailList",
