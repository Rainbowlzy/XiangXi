    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveContractManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ContractManagementList.html?table=ContractManagement"
                });
            },

			back(){
				window.location.href = site_gen+"ContractManagement.html?table=ContractManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetContractManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ContractManagement'
					vm = new Vue({
						el: "#ContractManagement",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'ContractManagement'
				vm = new Vue({
					el: "#ContractManagement",
					data: request||{},
					computed:{
