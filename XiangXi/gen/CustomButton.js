...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-CustomButton').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-CustomButton').on('shown.bs.modal',function(){
					$('.table_excel_modal-CustomButton .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=CustomButton", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"CustomButtonList.html"
					});
				})

				$('.table_excel_modal-CustomButton').modal('show');
			})
			$("#CustomButton").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveCustomButton", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"CustomButtonList.html?table=CustomButton"
                });
            },

			back(){
				window.location.href = site_gen+"CustomButton.html?table=CustomButton"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetCustomButtonEmpty", {}, function(data) {
					data = data||{};
					data.table = 'CustomButton'
					vm = new Vue({
						el: "#CustomButton",
						data: data||{},
						methods: method,
