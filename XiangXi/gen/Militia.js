    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveMilitia", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"MilitiaList.html?table=Militia"
                });
            },

			back(){
				window.location.href = site_gen+"Militia.html?table=Militia"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetMilitiaEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Militia'
					vm = new Vue({
						el: "#Militia",
