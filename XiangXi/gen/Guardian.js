    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveGuardian", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"GuardianList.html?table=Guardian"
                });
            },

			back(){
				window.location.href = site_gen+"Guardian.html?table=Guardian"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetGuardianEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Guardian'
					vm = new Vue({
						el: "#Guardian",
