    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveSystemConfiguration", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SystemConfigurationList.html?table=SystemConfiguration"
                });
            },

			back(){
				window.location.href = site_gen+"SystemConfiguration.html?table=SystemConfiguration"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSystemConfigurationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'SystemConfiguration'
					vm = new Vue({
						el: "#SystemConfiguration",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'SystemConfiguration'
				vm = new Vue({
					el: "#SystemConfiguration",
					data: request||{},
					computed:{
