    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveKeyPersonnel", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"KeyPersonnelList.html?table=KeyPersonnel"
                });
            },

			back(){
				window.location.href = site_gen+"KeyPersonnel.html?table=KeyPersonnel"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetKeyPersonnelEmpty", {}, function(data) {
					data = data||{};
					data.table = 'KeyPersonnel'
					vm = new Vue({
						el: "#KeyPersonnel",
						data: data||{},
						computed:{
