    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveAssets", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"AssetsList.html?table=Assets"
                });
            },

			back(){
				window.location.href = site_gen+"Assets.html?table=Assets"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetAssetsEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Assets'
					vm = new Vue({
						el: "#Assets",
