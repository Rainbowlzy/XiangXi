    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveThreeSessions", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ThreeSessionsList.html?table=ThreeSessions"
                });
            },

			back(){
				window.location.href = site_gen+"ThreeSessions.html?table=ThreeSessions"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetThreeSessionsEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ThreeSessions'
					vm = new Vue({
						el: "#ThreeSessions",
						data: data||{},
						computed:{
