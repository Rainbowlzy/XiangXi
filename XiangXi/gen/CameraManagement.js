...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-CameraManagement').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-CameraManagement').on('shown.bs.modal',function(){
					$('.table_excel_modal-CameraManagement .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=CameraManagement", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"CameraManagementList.html"
					});
				})

				$('.table_excel_modal-CameraManagement').modal('show');
			})
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveCameraManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"CameraManagementList.html?table=CameraManagement"
                });
            },

			back(){
				window.location.href = site_gen+"CameraManagement.html?table=CameraManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetCameraManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'CameraManagement'
					vm = new Vue({
						el: "#CameraManagement",
						data: data||{},
						methods: method,
				...
				request.table = 'CameraManagement'
				vm = new Vue({
					el: "#CameraManagement",
					data: request||{},
					methods: method,
