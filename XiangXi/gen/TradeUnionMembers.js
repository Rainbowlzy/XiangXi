    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveTradeUnionMembers", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"TradeUnionMembersList.html?table=TradeUnionMembers"
                });
            },

			back(){
				window.location.href = site_gen+"TradeUnionMembers.html?table=TradeUnionMembers"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetTradeUnionMembersEmpty", {}, function(data) {
					data = data||{};
					data.table = 'TradeUnionMembers'
					vm = new Vue({
						el: "#TradeUnionMembers",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'TradeUnionMembers'
				vm = new Vue({
					el: "#TradeUnionMembers",
					data: request||{},
					computed:{
