    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePartyAffairsGuide", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PartyAffairsGuideList.html?table=PartyAffairsGuide"
                });
            },

			back(){
				window.location.href = site_gen+"PartyAffairsGuide.html?table=PartyAffairsGuide"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPartyAffairsGuideEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PartyAffairsGuide'
					vm = new Vue({
						el: "#PartyAffairsGuide",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'PartyAffairsGuide'
				vm = new Vue({
					el: "#PartyAffairsGuide",
					data: request||{},
					computed:{
