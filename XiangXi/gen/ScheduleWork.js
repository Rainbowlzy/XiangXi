    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveScheduleWork", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ScheduleWorkList.html?table=ScheduleWork"
                });
            },

			back(){
				window.location.href = site_gen+"ScheduleWork.html?table=ScheduleWork"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetScheduleWorkEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ScheduleWork'
					vm = new Vue({
						el: "#ScheduleWork",
						data: data||{},
						computed:{
