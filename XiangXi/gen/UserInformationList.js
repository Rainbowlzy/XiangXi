...
$(document).ready(function(){
		...
		var vm = new Vue({
			...
			mounted:function(){
				...
				$('.excel-import-button-UserInformation').click(function () {
						...
						$('.table_excel_modal-UserInformation').on('shown.bs.modal',function(){
							...
							$(".excel_file").fileinput({
								language: 'zh', //设置语言
								uploadUrl: "/XiangXi/ExcelImport.ashx?fileType=UserInformation", //上传的地址
				...
				//表格传区域ID参数
				var url = "";
				$table.attr("data-url", "/XiangXi/DefaultHandler.ashx?method=getUserInformationList&data=" + JSON.stringify(GetRequest()) + "&");
				...
				var operateEvents = {
					...
					'click .edit': function(e, value, row, index) {
						...
						window.location.href = "/XiangXi/gen/UserInformation.html?table=UserInformation&data=" + encodeURIComponent(JSON.stringify(row));
					},
					//删除
					'click .remove': function(e, value, row, index) {
						...
						//后台删除
						$.call('DeleteUserInformation',row,function(data){
				...
				$table.bootstrapTable({
					...
					columns: [[
							...
							}, 
						{
							...
							{
								field: "UISubordinateDepartments",
								title: "所属部门",
											visible: false,
								sortable: true,
								align: "center"
							...
							{	
								formatter: imgFormatter('UIPhoto'),
								field: "UIPhoto",
								title: "照片",
											visible: false,
								sortable: true,
