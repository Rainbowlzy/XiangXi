...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-ConsolationRecord').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-ConsolationRecord').on('shown.bs.modal',function(){
					$('.table_excel_modal-ConsolationRecord .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=ConsolationRecord", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"ConsolationRecordList.html"
					});
				})

				$('.table_excel_modal-ConsolationRecord').modal('show');
			})
			$("#ConsolationRecord").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveConsolationRecord", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ConsolationRecordList.html?table=ConsolationRecord"
                });
            },

			back(){
				window.location.href = site_gen+"ConsolationRecord.html?table=ConsolationRecord"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetConsolationRecordEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ConsolationRecord'
					vm = new Vue({
						el: "#ConsolationRecord",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'ConsolationRecord'
				vm = new Vue({
					el: "#ConsolationRecord",
					data: request||{},
					methods: method,
