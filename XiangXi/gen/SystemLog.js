...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-SystemLog').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-SystemLog').on('shown.bs.modal',function(){
					$('.table_excel_modal-SystemLog .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=SystemLog", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"SystemLogList.html"
					});
				})

				$('.table_excel_modal-SystemLog').modal('show');
			})
			$("#SystemLog").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveSystemLog", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SystemLogList.html?table=SystemLog"
                });
            },

			back(){
				window.location.href = site_gen+"SystemLog.html?table=SystemLog"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSystemLogEmpty", {}, function(data) {
					data = data||{};
					data.table = 'SystemLog'
					vm = new Vue({
						el: "#SystemLog",
						data: data||{},
						methods: method,
