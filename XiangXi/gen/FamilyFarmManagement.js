...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-FamilyFarmManagement').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-FamilyFarmManagement').on('shown.bs.modal',function(){
					$('.table_excel_modal-FamilyFarmManagement .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=FamilyFarmManagement", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"FamilyFarmManagementList.html"
					});
				})

				$('.table_excel_modal-FamilyFarmManagement').modal('show');
			})
			$("#FamilyFarmManagement").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveFamilyFarmManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"FamilyFarmManagementList.html?table=FamilyFarmManagement"
                });
            },

			back(){
				window.location.href = site_gen+"FamilyFarmManagement.html?table=FamilyFarmManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetFamilyFarmManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'FamilyFarmManagement'
					vm = new Vue({
						el: "#FamilyFarmManagement",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'FamilyFarmManagement'
				vm = new Vue({
					el: "#FamilyFarmManagement",
					data: request||{},
					methods: method,
