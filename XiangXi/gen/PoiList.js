...
$(document).ready(function(){
		...
		var vm = new Vue({
			...
			mounted:function(){
				...
				$('.excel-import-button-Poi').click(function () {
						$('.table_add_modal').modal('hide');
						$('.table_excel_modal-Poi').on('shown.bs.modal',function(){
							$('.table_excel_modal-Poi .modal-body').html('<input class="excel_file input-file form-control" type="file">');
							$(".excel_file").fileinput({
								language: 'zh', //设置语言
								uploadUrl: "/XiangXi/ExcelImport.ashx?fileType=Poi", //上传的地址
							...
							$(".excel_file").on("fileuploaded",function(){
								window.location.href = site_gen+"PoiList.html"
							});
						})

						$('.table_excel_modal-Poi').modal('show');
					})
				
				var $table = $("#tablePoi")
				//表格传区域ID参数
				var url = "";
				$table.attr("data-url", "/XiangXi/DefaultHandler.ashx?method=getPoiList&data=" + JSON.stringify(GetRequest()) + "&");
				...
				var operateEvents = {
					...
					'click .edit': function(e, value, row, index) {
						...
						window.location.href = "/XiangXi/gen/Poi.html?table=Poi&data=" + encodeURIComponent(JSON.stringify(row));
					},
					//删除
					'click .remove': function(e, value, row, index) {
						...
						//后台删除
						$.call('DeletePoi',row,function(data){
