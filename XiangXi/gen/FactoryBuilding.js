    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveFactoryBuilding", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"FactoryBuildingList.html?table=FactoryBuilding"
                });
            },

			back(){
				window.location.href = site_gen+"FactoryBuilding.html?table=FactoryBuilding"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetFactoryBuildingEmpty", {}, function(data) {
					data = data||{};
					data.table = 'FactoryBuilding'
					vm = new Vue({
						el: "#FactoryBuilding",
						data: data||{},
						computed:{
