    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveVideoPointInformation", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"VideoPointInformationList.html?table=VideoPointInformation"
                });
            },

			back(){
				window.location.href = site_gen+"VideoPointInformation.html?table=VideoPointInformation"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetVideoPointInformationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'VideoPointInformation'
					vm = new Vue({
						el: "#VideoPointInformation",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'VideoPointInformation'
				vm = new Vue({
					el: "#VideoPointInformation",
					data: request||{},
					computed:{
