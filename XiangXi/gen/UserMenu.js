    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveUserMenu", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"UserMenuList.html?table=UserMenu"
                });
            },

			back(){
				window.location.href = site_gen+"UserMenu.html?table=UserMenu"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetUserMenuEmpty", {}, function(data) {
					data = data||{};
					data.table = 'UserMenu'
					vm = new Vue({
						el: "#UserMenu",
