    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePoi", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PoiList.html?table=Poi"
                });
            },

			back(){
				window.location.href = site_gen+"Poi.html?table=Poi"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPoiEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Poi'
					vm = new Vue({
						el: "#Poi",
