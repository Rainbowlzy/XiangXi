...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-LogonLog').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-LogonLog').on('shown.bs.modal',function(){
					$('.table_excel_modal-LogonLog .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=LogonLog", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"LogonLogList.html"
					});
				})

				$('.table_excel_modal-LogonLog').modal('show');
			})
			$("#LogonLog").show();
        };
        var method = {
            ...
            save() {
                ...
                $.call("SaveLogonLog", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"LogonLogList.html?table=LogonLog"
                });
            },

			back(){
				window.location.href = site_gen+"LogonLog.html?table=LogonLog"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetLogonLogEmpty", {}, function(data) {
					data = data||{};
					data.table = 'LogonLog'
					vm = new Vue({
						el: "#LogonLog",
