...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-SanitationActivities').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-SanitationActivities').on('shown.bs.modal',function(){
					$('.table_excel_modal-SanitationActivities .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=SanitationActivities", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"SanitationActivitiesList.html"
					});
				})

				$('.table_excel_modal-SanitationActivities').modal('show');
			})
			$("#SanitationActivities").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveSanitationActivities", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"SanitationActivitiesList.html?table=SanitationActivities"
                });
            },

			back(){
				window.location.href = site_gen+"SanitationActivities.html?table=SanitationActivities"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetSanitationActivitiesEmpty", {}, function(data) {
					data = data||{};
					data.table = 'SanitationActivities'
					vm = new Vue({
						el: "#SanitationActivities",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'SanitationActivities'
				vm = new Vue({
					el: "#SanitationActivities",
					data: request||{},
					methods: method,
