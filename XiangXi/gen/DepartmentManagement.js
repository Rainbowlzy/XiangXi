...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-DepartmentManagement').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-DepartmentManagement').on('shown.bs.modal',function(){
					$('.table_excel_modal-DepartmentManagement .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=DepartmentManagement", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"DepartmentManagementList.html"
					});
				})

				$('.table_excel_modal-DepartmentManagement').modal('show');
			})
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveDepartmentManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"DepartmentManagementList.html?table=DepartmentManagement"
                });
            },

			back(){
				window.location.href = site_gen+"DepartmentManagement.html?table=DepartmentManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetDepartmentManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'DepartmentManagement'
					vm = new Vue({
						el: "#DepartmentManagement",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'DepartmentManagement'
				vm = new Vue({
					el: "#DepartmentManagement",
					data: request||{},
					methods: method,
