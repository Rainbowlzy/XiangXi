...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-UserMessageQueue').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-UserMessageQueue').on('shown.bs.modal',function(){
					$('.table_excel_modal-UserMessageQueue .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=UserMessageQueue", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"UserMessageQueueList.html"
					});
				})

				$('.table_excel_modal-UserMessageQueue').modal('show');
			})
			$("#UserMessageQueue").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveUserMessageQueue", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"UserMessageQueueList.html?table=UserMessageQueue"
                });
            },

			back(){
				window.location.href = site_gen+"UserMessageQueue.html?table=UserMessageQueue"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetUserMessageQueueEmpty", {}, function(data) {
					data = data||{};
					data.table = 'UserMessageQueue'
					vm = new Vue({
						el: "#UserMessageQueue",
						data: data||{},
						methods: method,
				...
				request.table = 'UserMessageQueue'
				vm = new Vue({
					el: "#UserMessageQueue",
					data: request||{},
					methods: method,
