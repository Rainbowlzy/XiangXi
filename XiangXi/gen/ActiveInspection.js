    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveActiveInspection", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ActiveInspectionList.html?table=ActiveInspection"
                });
            },

			back(){
				window.location.href = site_gen+"ActiveInspection.html?table=ActiveInspection"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetActiveInspectionEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ActiveInspection'
					vm = new Vue({
						el: "#ActiveInspection",
						data: data||{},
						computed:{
				...
				request.table = 'ActiveInspection'
				vm = new Vue({
					el: "#ActiveInspection",
					data: request||{},
					computed:{
