    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SavePartyFeeManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"PartyFeeManagementList.html?table=PartyFeeManagement"
                });
            },

			back(){
				window.location.href = site_gen+"PartyFeeManagement.html?table=PartyFeeManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetPartyFeeManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'PartyFeeManagement'
					vm = new Vue({
						el: "#PartyFeeManagement",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'PartyFeeManagement'
				vm = new Vue({
					el: "#PartyFeeManagement",
					data: request||{},
					computed:{
