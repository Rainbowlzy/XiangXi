    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveLoginRecord", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"LoginRecordList.html?table=LoginRecord"
                });
            },

			back(){
				window.location.href = site_gen+"LoginRecord.html?table=LoginRecord"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetLoginRecordEmpty", {}, function(data) {
					data = data||{};
					data.table = 'LoginRecord'
					vm = new Vue({
						el: "#LoginRecord",
						data: data||{},
						computed:{
