    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveInvestors", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"InvestorsList.html?table=Investors"
                });
            },

			back(){
				window.location.href = site_gen+"Investors.html?table=Investors"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetInvestorsEmpty", {}, function(data) {
					data = data||{};
					data.table = 'Investors'
					vm = new Vue({
						el: "#Investors",
						data: data||{},
						computed:{
