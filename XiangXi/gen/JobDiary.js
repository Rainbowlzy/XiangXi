    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveJobDiary", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"JobDiaryList.html?table=JobDiary"
                });
            },

			back(){
				window.location.href = site_gen+"JobDiary.html?table=JobDiary"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetJobDiaryEmpty", {}, function(data) {
					data = data||{};
					data.table = 'JobDiary'
					vm = new Vue({
						el: "#JobDiary",
