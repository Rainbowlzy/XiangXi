    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveConvenientLife", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ConvenientLifeList.html?table=ConvenientLife"
                });
            },

			back(){
				window.location.href = site_gen+"ConvenientLife.html?table=ConvenientLife"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetConvenientLifeEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ConvenientLife'
					vm = new Vue({
						el: "#ConvenientLife",
						data: data||{},
						computed:{
