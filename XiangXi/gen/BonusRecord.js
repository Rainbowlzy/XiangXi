    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveBonusRecord", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"BonusRecordList.html?table=BonusRecord"
                });
            },

			back(){
				window.location.href = site_gen+"BonusRecord.html?table=BonusRecord"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetBonusRecordEmpty", {}, function(data) {
					data = data||{};
					data.table = 'BonusRecord'
					vm = new Vue({
						el: "#BonusRecord",
						data: data||{},
						computed:{
