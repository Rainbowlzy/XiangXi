...
require([
    ...
    $(document).ready(function() {
        ...
        var created = function () {
			...
			$('.excel-import-button-TouristInformation').click(function () {
				$('.table_add_modal').modal('hide');
				$('.table_excel_modal-TouristInformation').on('shown.bs.modal',function(){
					$('.table_excel_modal-TouristInformation .modal-body').html('<input class="excel_file input-file form-control" type="file">');
					$(".excel_file").fileinput({
						language: 'zh', //设置语言
						uploadUrl: "../Data/ExcelImport.ashx?fileType=TouristInformation", //上传的地址
					...
					$(".excel_file").on("fileuploaded",function(){
						window.location.href = site_gen+"TouristInformationList.html"
					});
				})

				$('.table_excel_modal-TouristInformation').modal('show');
			})
			$("#TouristInformation").show();
        };
        var method = {
			log(o){
				(o)
            ...
            save() {
                ...
                $.call("SaveTouristInformation", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"TouristInformationList.html?table=TouristInformation"
                });
            },

			back(){
				window.location.href = site_gen+"TouristInformation.html?table=TouristInformation"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetTouristInformationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'TouristInformation'
					vm = new Vue({
						el: "#TouristInformation",
						data: data||{},
						methods: method,
						mounted:created
					});
				...
				request.table = 'TouristInformation'
				vm = new Vue({
					el: "#TouristInformation",
					data: request||{},
					methods: method,
