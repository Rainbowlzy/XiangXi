    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveScheduleManagement", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"ScheduleManagementList.html?table=ScheduleManagement"
                });
            },

			back(){
				window.location.href = site_gen+"ScheduleManagement.html?table=ScheduleManagement"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetScheduleManagementEmpty", {}, function(data) {
					data = data||{};
					data.table = 'ScheduleManagement'
					vm = new Vue({
						el: "#ScheduleManagement",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'ScheduleManagement'
				vm = new Vue({
					el: "#ScheduleManagement",
					data: request||{},
					computed:{
