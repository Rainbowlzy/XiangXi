    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveUserInformation", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"UserInformationList.html?table=UserInformation"
                });
            },

			back(){
				window.location.href = site_gen+"UserInformation.html?table=UserInformation"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetUserInformationEmpty", {}, function(data) {
					data = data||{};
					data.table = 'UserInformation'
					vm = new Vue({
						el: "#UserInformation",
						data: data||{},
						computed:{
