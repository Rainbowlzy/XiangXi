    ...
    $(document).ready(function() {
        ...
        var method = {
            ...
            save() {
                ...
                $.call("SaveBeautifulCountryside", datum, function(data) {
					if(!data) return bootstrap_alert('返回数据为空')
					if(!data.success) return bootstrap_alert(data.message)
					window.location.href = site_gen+"BeautifulCountrysideList.html?table=BeautifulCountryside"
                });
            },

			back(){
				window.location.href = site_gen+"BeautifulCountryside.html?table=BeautifulCountryside"
			},
			...
			if (!request || request.id === undefined) {
				$.call("GetBeautifulCountrysideEmpty", {}, function(data) {
					data = data||{};
					data.table = 'BeautifulCountryside'
					vm = new Vue({
						el: "#BeautifulCountryside",
						data: data||{},
						computed:{
							request:function(){
								return GetRequest();
				...
				request.table = 'BeautifulCountryside'
				vm = new Vue({
					el: "#BeautifulCountryside",
					data: request||{},
					computed:{
